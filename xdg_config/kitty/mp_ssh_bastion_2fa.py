...
def main(args):
    ...
    while answer is None:
        ...
        if '401: Authentication required' in otp:
            # we have a token, but it is expired; retry
            os.environ.pop(session_env_var)
            token = None
        else:
            answer = {
                'token': token,
...
def handle_result(args, payload, target_window_id, boss):
    ...
    # for it in the current shell and also in new shells.
